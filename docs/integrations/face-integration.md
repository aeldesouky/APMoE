...
* Image must be resized to **200 Ã— 200**
...
The face model is a **regressor**: it returns a single age value, not class probabilities. In the APMoE framework, `FaceAgeExpert` sets per-expert `confidence` to **`-1.0`**, which means **confidence is not reported** (not a score in `[0, 1]`). Aggregators treat `-1` as â€œno self-reported confidenceâ€ when combining experts; the final prediction `confidence` stays in `[0.0, 1.0]`. Use explicit `aggregation.weights` in config if you need to control how the face expert blends with others.
...
```
Image Input â†’ Preprocessing â†’ Model â†’ Age Output
```
...
```
User Input
   â”œâ”€â”€ Image â†’ Age Model
   â””â”€â”€ Keystrokes â†’ Behavior Model

Final Decision Layer â†’ Output
```
